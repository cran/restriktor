summary.restriktor <- function(object, bootCIs = TRUE, bty = "perc", 
  ...
  wt.bar <- z$wt.bar
  ## compute goric
  if (GORIC && !(attr(wt.bar, "wt.bar.method") == "none")) {
    ...
    if (attr(wt.bar, "wt.bar.method") == "boot") {
      ...
      # unconstrained case
    } else if (attr(wt.bar, "wt.bar.method") == "pmvnorm" && all(c(Amat) == 0)) {
      PT <- p + 1
    } else if (attr(wt.bar, "wt.bar.method") == "pmvnorm") {
