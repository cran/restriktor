...
con_pvalue_boot_parametric <- function(model, Ts.org, 
  ...
  bvec <- model$rhs
  if (any(abs(bvec) > 0)) {
    stop("shifted/relocated convex cones are not supported (yet).",
         "\n       The p-value can be computed using the mixing weights,",
         "\n       mix.weights = \"pmvnorm\" or \"boot\".")
  }
  meq  <- model$neq
  ...
  Ts.boot <- vector("numeric", R)
   fn <- function(b) {
    ...
    ystar <- p.distr(n) 
    # TO DO: shift y in case R\theta >= r
